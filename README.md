...
## 👀 更新日志
### 2026.09.25 推荐回测支持多持有期与全量历史覆盖
- 新增 5 个内置定时任务「推荐回测(3日/5日/10日/20日/30日)」，交易日 18:30~19:10 错峰自动执行；当天未运行到该时刻，下次启动软件后自动补跑
- 回测按「推荐 + 持有期」分别核算，同一只票的不同持有期互不覆盖；「推荐回测统计」页与 AI 推荐列表、提示词模板回测弹窗均新增持有期切换（3/5/10/20/30 或全部周期），统计、明细与「执行回测」都按所选持有期
- 回测改为**全量覆盖**：取消单次条数上限（改用时间预算分批），K 线取数窗口从 300 根放宽到 5000 根（此前超过 300 个交易日的老推荐永远取不到推荐日），推荐时间字段缺失的存量记录自动退回创建时间参与回测
- 沪深300 基准在整轮回测中只拉取一次，全量回测时不再逐条重复请求
- 「推荐回测统计」页顶部新增「待回测 N 条」提示，为 0 即表示该持有期的历史推荐已全部回测完成
- 定时任务管理页支持选择「推荐回测」任务类型并配置持有期参数
### 2026.09.23 信号监控监控池改为全市场选股 + 收益统计口径修正
- 监控池添加股票改为与「股票自选」同款的全市场模糊搜索（A股 / 指数 / 港美股 / 场内基金，不限自选股），输入即防抖在线搜索；同时移除手输代码通道，选股入口只保留搜索下拉框
- 修复收益统计「持仓中」恒为 0：整体统计改由按股票明细汇总得出，与明细必然一致
- 修复自定义日期区间漏算结束日：结束日按当日收盘后纳入统计，与「今日 / 3日 / 5日 / 10日 / 20日」快捷区间口径一致
- 收益统计成交价改用信号所在 K 线的**收盘价**（此前用的是图上箭头坐标——买点取最低价、卖点取最高价，会系统性高估收益）
- 修复区间起点丢弃平仓笔：区间内平仓的卖点会回看区间之前最多 120 天的买点来配对，避免「前几天买入、今天卖出」的收益漏算；「持仓中」只统计区间内开仓且尚未平仓的笔
### 2026.09.23 修复钉钉/飞书推送收不到消息
- 修复钉钉推送「提示发送成功、群里却收不到」：此前前端拼的消息字符串被原样当作请求体发出，消息体非法时钉钉返回 40035 却被当成成功；现统一校验并规范化消息体，同时解析返回的 errcode，失败时给出真实原因
- 钉钉与飞书推送默认 @所有人；群或机器人未开放该权限时（钉钉 450103 / 飞书发送被拒）自动去掉 @ 重发一次，确保消息必达
- 设置页「发送测试通知」改为使用页面上当前填写的机器人地址（飞书同时使用页面上填写的签名密钥），不用先保存即可测试，也不再误发到已保存的旧地址
- 飞书测试消息在配置了签名密钥时自动附带 timestamp + sign，避免开启签名校验后测试失败
### 2026.09.23 后台信号提示音支持语音播报股票名称
- 后台信号监控「提醒方式」新增「语音报名称」开关：每段提示音放完再用系统语音念一句「股票名称 + 方向」（如「贵州茅台 买点」），多只票同时命中时不用回到屏幕也能分辨是哪一只
- 语音走系统语音合成（Web Speech API），无需联网；优先选用中文语音，系统无中文语音包时自动降级为只播提示音并给出提示
- 打开开关即试听一句示例；开启语音后连播间隔自动放宽，避免下一只票的提示音打断上一句
- 名称缺失时退化为去掉市场前后缀的代码播报，避免被逐字符读出
### 2026.09.23 修复提示音静默与信号监控体验优化
- 修复买卖点 / T确 / 后台监控提示音「永久无声」：音频上下文未预热成功时不再直接放弃播放，改为播放时自愈创建并尝试恢复，系统休眠或切换音频设备导致挂起后也能自动恢复
- 音频预热由「一次手势即注销」改为常驻手势恢复，避免首次交互未被识别后再也无法出声
- 点击提示音开关试听时若音频未就绪，给出当前状态提示与排查方向（先点击窗口任意位置、检查系统音量与音量合成器）
- 补全 TEMA 转折信号在流水中的收盘价，统一时间显示格式为 `yyyy-MM-dd HH:mm`
- 新增《后台买卖点信号监控》功能介绍与使用说明文档（docs/后台买卖点信号监控.md / .pdf）
### 2026.09.22 新增后台买卖点信号监控（VIP2）
- 新增「信号监控」浮标 + 抽屉面板：后台常驻扫描监控池，命中买卖点（9 路共振）或 TEMA 转折（T确）即推送提醒，监控池与自选股解耦
- 每只票可单独勾选多个周期（如 日K + 60分），各周期独立出信号；未单独设置时继承全局默认周期
- 扫描节拍放在 Go 侧（每 60 秒），窗口最小化也持续提醒；取数口径与 K 线图完全一致，避免门控静默失效导致信号与图上箭头对不上
- 提醒渠道支持系统通知 / 飞书 / 钉钉，应用内提示音可开关，同一根 K 线不重复提醒（图表与后台共享同一套抢占式去重）
- 信号流水改存后端 SQLite，支持按代码/名称 + K 线时间区间筛选与分页浏览，每条可一键打开 K 线图核对
- 新增信号收益统计：按「买点开仓、卖点平仓」逐笔配对，统计累计收益、胜率、笔数与持仓中，支持今日 / 3日 / 5日 / 10日 / 20日快捷区间与自定义区间，并可按股票查看明细
- 本功能需 VIP2 及以上赞助用户，权限不足时点击浮标提示升级且不打开面板，后台不启动扫描
